            ...
            <div className="relative max-w-7xl mx-auto">
                ...
                <div className="mt-12 max-w-lg mx-auto grid gap-7 lg:grid-cols-3 lg:max-w-none">
                    {partners.map((partner) => (
                        <div key={id + partner.companyName}
                             className="flex flex-col rounded-lg shadow-lg overflow-hidden">
                            <div className="flex-shrink-0">
                                <a href={`/partners/${partner.companyName}`}>
                                    <img className="h-48 w-full p-6"
                                         src={`${process.env.NEXT_PUBLIC_API_ENDPOINT}/Uploads/Partners/${partner.thumbnailName}`}
                                         alt=""/>
                                </a>
                            </div>
                            <div className="flex-1 bg-white p-6 flex flex-col justify-between">
                                <div className="flex-1">
                                    <a href={`/partners/${partner.companyName}`} className="block mt-2 hover:underline">
